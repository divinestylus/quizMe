...
function displayQuestion(qObj) {
  pageTrackerEl.textContent = `${questionCount + 1}/${pythonQuestionsBank.length}`;
...
nextBtnEl.addEventListener('click', function () {
  enablePrevBtn();
  if (currentIndex < pythonQuestionsBank.length) {
    questionCount++;
    currentIndex++;
    question = pythonQuestionsBank[currentIndex];
    optionsContainerEl.innerHTML = "";
    if (currentIndex >= pythonQuestionsBank.length - 1) {
    ...
    displayQuestion(pythonQuestionsBank);
    checkAnswer();
  }
})
...
function checkAnswer() {
  ...
  let correctAnswer = pythonQuestionsBank[currentIndex].correctAnswer;
