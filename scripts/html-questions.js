...
function displayQuestion(qObj) {
  pageTrackerEl.textContent = `${questionCount + 1}/${htmlQuestionsBank.length}`;
...
nextBtnEl.addEventListener('click', function () {
  enablePrevBtn();
  if (currentIndex < htmlQuestionsBank.length) {
    questionCount++;
    currentIndex++;
    question = htmlQuestionsBank[currentIndex];
    optionsContainerEl.innerHTML = "";
    if (currentIndex >= htmlQuestionsBank.length - 1) {
    ...
    displayQuestion(htmlQuestionsBank);
    checkAnswer();
  }
})
