...
function displayQuestion(qObj) {
  pageTrackerEl.textContent = `${questionCount + 1}/${jsQuestionsBank.length}`;
...
nextBtnEl.addEventListener('click', function () {
  enablePrevBtn();
  if (currentIndex < jsQuestionsBank.length) {
    questionCount++;
    currentIndex++;
    question = jsQuestionsBank[currentIndex];
    optionsContainerEl.innerHTML = "";
    if (currentIndex >= jsQuestionsBank.length - 1) {
    ...
    displayQuestion(jsQuestionsBank);
    checkAnswer();
  }
})
...
function checkAnswer() {
  ...
  let correctAnswer = jsQuestionsBank[currentIndex].correctAnswer;
