...
function displayQuestion(qObj) {
  pageTrackerEl.textContent = `${questionCount + 1}/${nodejsQuestionsBank.length}`;
...
nextBtnEl.addEventListener('click', function () {
  enablePrevBtn();
  if (currentIndex < nodejsQuestionsBank.length) {
    questionCount++;
    currentIndex++;
    question = nodejsQuestionsBank[currentIndex];
    optionsContainerEl.innerHTML = "";
    if (currentIndex >= nodejsQuestionsBank.length - 1) {
    ...
    displayQuestion(nodejsQuestionsBank);
    checkAnswer();
  }
})
...
function checkAnswer() {
  ...
  let correctAnswer = nodejsQuestionsBank[currentIndex].correctAnswer;
